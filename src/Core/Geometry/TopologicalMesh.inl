...
void copyAttribToWedgeData( const IndexedGeometry<U>& mesh,
                            unsigned int vindex,
...
template <typename T>
void copyMeshToWedgeData( const IndexedGeometry<T>& mesh,
                          unsigned int vindex,
...
template <typename T>
void TopologicalMesh::initWithWedge( const IndexedGeometry<T>& triMesh ) {

    // LOG( logINFO ) << "TopologicalMesh: load mesh with " << triMesh.getIndices().size()
    ...
    for ( size_t i = 0; i < triMesh.vertices().size(); ++i )
    {
        ...
        /// the newly added wedge is not referenced yet.
        auto idx = m_wedges.m_data.size() - 1;
        m_wedges.del( idx );
    }
    ...
    for ( unsigned int i = 0; i < num_triangles; i++ )
    {
        ...
        // LOG( logINFO ) << "TopologicalMesh: add face "
        // << i << " with " << num_vert << " vertices ";
        ...
        for ( size_t j = 0; j < num_vert; ++j )
        {
            ...
            face_wedges[j]      = WedgeIndex {int( inMeshVertexIndex )};
        }
...
inline void TopologicalMesh::copyAllPropsFromFace( FaceHandle fh,
                                                   HalfedgeHandle heh,
...
inline void
TopologicalMesh::interpolateAllPropsOnFaces( FaceHandle fh,
                                             OpenMesh::FPropHandleT<Normal> normalProp ) {
